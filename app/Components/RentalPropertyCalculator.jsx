...
const RentalPropertyCalculator = () => {
  ...
  }, [propertyPrice, downPaymentPercentage, interestRate, mortgageTerm, monthlyRent, 
      propertyTaxType, propertyTaxRate, propertyTaxAmount,
      insuranceType, insuranceRate, insuranceAmount,
      maintenanceType, maintenanceRate, maintenanceAmount,
      vacancyType, vacancyRate, vacancyAmount, condoFees]);
  ...
  const ResultsSection = () => {
    ...
    return (
      <Card className="">
        ...
        <h3 className="font-semibold">Monthly Mortgage Payment : ${monthlyMortgage.toFixed(2)}</h3>
        <ul className="list-disc list-inside text-sm">
          <li>Loan Amount: ${(propertyPrice - (propertyPrice * downPaymentPercentage / 100)).toFixed(2)}</li>
          <li>Monthly Interest Rate: {(interestRate / 12 / 100).toFixed(4)}%</li>
          <li>Term in Months: {mortgageTerm * 12}</li>
        </ul>

        <div>
        <h3 className="font-semibold">Total Monthly Costs : ${totalMonthlyCosts.toFixed(2)}</h3>
        <p className="text-xs text-gray-600 py-1">
          Calculated as: Monthly Mortgage + Property Tax + Insurance + Maintenance + Vacancy + Condo Fees
        </p>
        <ul className="list-disc list-inside text-sm">
          <li>Monthly Mortgage: ${monthlyMortgage.toFixed(2)}</li>
          <li>Property Tax: ${(propertyTaxType === 'percentage' 
              ? (propertyPrice * propertyTaxRate / 100 / 12) 
              : (propertyTaxAmount / 12)).toFixed(2)}</li>
          <li>Insurance: ${(insuranceType === 'percentage'
              ? (propertyPrice * insuranceRate / 100 / 12)
              : (insuranceAmount / 12)).toFixed(2)}</li>
          <li>Maintenance: ${(maintenanceType === 'percentage'
              ? (propertyPrice * maintenanceRate / 100 / 12)
              : (maintenanceAmount / 12)).toFixed(2)}</li>
          <li>Vacancy: ${(vacancyType === 'percentage'
              ? (monthlyRent * vacancyRate / 100)
              : (vacancyAmount / 12)).toFixed(2)}</li>
              <li>Condo Fees: ${condoFees.toFixed(2)}</li>
        </ul>
      </div>

      <div>
        <h3 className="font-semibold">Net Cash Flow : ${netCashFlow.toFixed(2)}</h3>
...
<Select className="" value={growthPeriod} onValueChange={setGrowthPeriod}>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    id={propertyTaxType === 'percentage' ? "propertyTaxRate" : "propertyTaxAmount"}
                    type="number"
                    value={propertyTaxType === 'percentage' ? propertyTaxRate : propertyTaxAmount}
                    onChange={(e) => propertyTaxType === 'percentage' ? setPropertyTaxRate(Number(e.target.value)) : setPropertyTaxAmount(Number(e.target.value))}
                    className=""
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    id={insuranceType === 'percentage' ? "insuranceRate" : "insuranceAmount"}
                    type="number"
                    value={insuranceType === 'percentage' ? insuranceRate : insuranceAmount}
                    onChange={(e) => insuranceType === 'percentage' ? setInsuranceRate(Number(e.target.value)) : setInsuranceAmount(Number(e.target.value))}
                    className=""
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    id={maintenanceType === 'percentage' ? "maintenanceRate" : "maintenanceAmount"}
                    type="number"
                    value={maintenanceType === 'percentage' ? maintenanceRate : maintenanceAmount}
                    onChange={(e) => maintenanceType === 'percentage' ? setMaintenanceRate(Number(e.target.value)) : setMaintenanceAmount(Number(e.target.value))}
                    className=""
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <Input
                      id={vacancyType === 'percentage' ? "vacancyRate" : "vacancyAmount"}
                      type="number"
                      value={vacancyType === 'percentage' ? vacancyRate : vacancyAmount}
                      onChange={(e) => vacancyType === 'percentage' ? setVacancyRate(Number(e.target.value)) : setVacancyAmount(Number(e.target.value))}
                      className=""
                    />
                  </div>
